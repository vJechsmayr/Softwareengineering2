...
public class ShapeModel {
	...
	ArrayList<IUndoRedoAction> actionHistory;
	int actionIndex;
	...
	public void addShape(Shape s) {
		...
		AddShapeAction action = new AddShapeAction(s, this);
		AddAction(action);
	}

	/**
	 * Remove all redone Actions when there is a new one for a correct Logic 
	 * 
	 * @param action
	 *            IUndoRedoAction action which should be added to the History
	 */
	...
	/**
	 * undo-Method for the model
	 */
	public void undo()
	{
		if(actionIndex < actionHistory.size())
		{
			actionHistory.get(actionIndex).undo();
			actionIndex++;
		}
	}
	
	/**
	 * redo-Method for the model
	 */
	public void redo()
	{
		if(actionIndex > 0)
		{
			actionHistory.get(actionIndex).redo();
			actionIndex--;
		}
	}
